...
6. If existing-system understanding is reusable beyond the current feature, use `protocols/study-surface.md` before promoting only change-specific facts into planning.
7. After classification, choose exactly one planning protocol:
   - `planning-greenfield.md`
   - `planning-existing.md`
   - `planning-hybrid.md`
8. Resolve the feature folder before writing:
   - existing project: write under `<SKILLDOCS_DIR>/<feature-name>/`
   - new project: bootstrap project root plus first feature folder
9. Never create a second feature `task.md` at the project root.
