...
## 5. Choice-Against-Alternatives (Exemplar-Derived Standards)

When deriving standards from an exemplar's real code (a designated style-baseline author), do not just describe what they wrote. For each decision point:

1. State the goal the code had to achieve.
2. Enumerate the 2-3 idiomatic alternatives a developer would realistically consider (languages like Ruby offer many ways to do one thing — the alternatives ARE the teaching material).
3. Quote the exemplar's actual choice verbatim (sha + file).
4. State WHY that choice wins under the project's constraints — grounded in evidence (commit message, comment, surrounding architecture). Mark unsupported reasoning as inferred.

The WHY becomes the executable rule: "when you face this decision point, prefer X unless condition Z." Recent commits weigh more than old ones — exemplars evolve, and the standard should capture their current judgment, not their history. Quantified claims about the exemplar (commit counts, ratios) must be independently re-verified before entering a standard.
