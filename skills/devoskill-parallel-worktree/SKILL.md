...
description: Runtime sidecar and concurrent-worktree support module for DevoSkill. Use when one or more task worktrees also need isolated app containers, hostnames, shared-service wiring, or coordination across concurrent agents/sessions. Ordinary Git isolation is owned by the git-worktree-isolation protocol.
---

# DevoSkill Parallel Worktree

Use this skill when the immediate need is **runtime isolation for task worktrees**, beyond the dedicated Git worktree required by `protocols/git-worktree-isolation.md`. Typical triggers:
...
Do **not** preload this skill for ordinary single-task work that needs only Git isolation. Apply `git-worktree-isolation.md` and stay in the task worktree without creating a sidecar.
...
1. **Confirm runtime isolation is actually needed.** If the task only needs a Git worktree, apply `protocols/git-worktree-isolation.md` and reroute back to Development. Do not edit `main_checkout`.
...
- **Read-only overlay mounts.** Files in `overlay_files` are bind-mounted from `main_checkout` into the sidecar. Do not edit them in either checkout as an implementation shortcut. If local runtime configuration must change, make it an explicit work-unit prerequisite or update the sidecar/config design without disturbing the primary checkout.
...
- Ordinary task worktree isolation: `{DEVOSKILL_ROOT}/skills/devoskill/protocols/git-worktree-isolation.md`.
...
5. **Read-only overlays stay untouched.** Runtime-config changes require an explicit work-unit path; never edit the primary checkout as a sidecar shortcut.
