...
Only use this skill when the canonical mapping state is missing or the task is explicitly about workspace setup. If `{DEVOSKILL_ROOT}/skills/devoskill/config/workspace-map.local.json` already exists and the task is not a workspace repair, return to the active work mode instead of reading more setup material.

## Load Order
1. Read `{DEVOSKILL_ROOT}/skills/devoskill/config/workspace-map.example.json`
2. Read `{DEVOSKILL_ROOT}/skills/devoskill/protocols/workspace-setup.md`
...
## Required Behavior
- Resolve the mapped SkillDocs base path from `{DEVOSKILL_ROOT}/skills/devoskill/config/workspace-map.local.json` when present.
